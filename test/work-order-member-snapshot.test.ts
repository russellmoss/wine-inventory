...
type FakeTask = { id: string; title: string; vesselGroupId: string | null; memberSnapshotAt: Date | null; memberSnapshot?: unknown };
...
describe("freeze at issue", () => {
  it("writes the current membership onto the task", async () => {
    const tasks: FakeTask[] = [{ id: "t1", title: "Top rack 14", vesselGroupId: "g1", memberSnapshotAt: null }];
  ...
  it("freezes several tasks against different groups independently (per-task, not per-work-order)", async () => {
    ...
    const tasks: FakeTask[] = [
      { id: "t1", title: "Top rack 14", vesselGroupId: "g1", memberSnapshotAt: null },
      { id: "t2", title: "SO2 the new oak", vesselGroupId: "g2", memberSnapshotAt: null },
    ];
