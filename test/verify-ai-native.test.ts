  ...
  it("INTERNAL is permanent — exempt even with MAX_ALLOWED=0", () => {
    ...
    const { violations } = run({
      internal: {
        ...
        "src/lib/owner/owner-core.ts": { owner: "x", reason: "plan 093 F3 gap (test-local)" },
        "src/lib/grower/grower-core.ts": { owner: "x", reason: "plan 093 F3 gap (test-local)" },
        "src/lib/plantingArea/migration-core.ts": { owner: "x", reason: "VI-P1 reviewed setup flow (test-local)" },
        "src/lib/weather/ingest-core.ts": { owner: "x", reason: "VI-P8 weather refresh mechanism (test-local)" },
        "src/lib/weather/alert-core.ts": { owner: "x", reason: "VI-P8 sweep frost/heat alert (test-local)" },
        "src/lib/weather/forecast-ingest-core.ts": { owner: "x", reason: "plan 096 forecast refresh mechanism (test-local)" },
        // S3a: covered by the S11 write tool + S5a→S9 read tool when those phases land (test-local).
        "src/lib/spray/record-core.ts": { owner: "x", reason: "S3a — S11 write tool pending (test-local)" },
        "src/lib/spray/correction-core.ts": { owner: "x", reason: "S3a — S11 write tool pending (test-local)" },
        "src/lib/spray/drying-override-core.ts": { owner: "x", reason: "S3a — S11 write tool pending (test-local)" },
        "src/lib/harvest/planned-harvest-core.ts": { owner: "x", reason: "S3a — S11 write tool pending (test-local)" },
        "src/lib/spray/legacy-mapping-core.ts": { owner: "x", reason: "S3a — review-surface + S11 (test-local)" },
      },
      gapAllowlist: {},
      maxAllowed: 0,
    });
