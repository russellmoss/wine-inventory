...
export function decideAdmission(
  ...
  // isDenied, not a bare startsWith: `/grapes/hops` must not slip past a `/grapes/hops/` deny.
  if (isDenied(cfg, path)) return REFUSE;
...
export async function crawlSource(
  ...
  const summary: CrawlSummary = {
    ...
    notModified: 0, skippedRobots: 0, skippedType: 0, skippedRedirect: 0, skippedSoftNotFound: 0, skippedRedirectDenied: 0, skippedChallenge: 0, errors: 0, candidates: 0,
  };
...
export async function crawlWithFollowing(
  ...
  for (const key of sourceKeys) {
    ...
    summaries[key] = { source: key, discovered: 0, fetched: 0, documents: 0, notModified: 0, skippedRobots: 0, skippedType: 0, skippedRedirect: 0, skippedSoftNotFound: 0, skippedRedirectDenied: 0, skippedChallenge: 0, errors: 0, candidates: 0 };
