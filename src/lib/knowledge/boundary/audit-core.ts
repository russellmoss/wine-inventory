// SKB Unit 2 - the pure arithmetic behind verify:kb-boundary.
//
 ...
 * unaudited = we could not re-read its raw bytes, so we know nothing.
 * empty = the document has zero stored chunks - the ingest gate already fired (or the section
 * filter dropped everything) and nothing from it is retrievable, so there is nothing to audit and it
 * is never fetched. See verify-kb-boundary.ts auditLive.
 */
export type AuditVerdict = BoundaryVerdict | "unaudited" | "empty";
...
export interface SourceTally {
  ...
  unaudited: number;
  /** Documents with zero stored chunks - nothing retrievable, never fetched. See empty on AuditVerdict. */
  empty: number;
}
...
export interface BoundaryAuditSummary {
  ...
  /** The number D3 close-out decides against: flagged documents across the report-only census. */
...
export function summarizeBoundaryAudit(rows: AuditRow[]): BoundaryAuditSummary {
  ...
  for (const row of rows) {
    ...
    if (!t) {
      t = { sourceKey: row.sourceKey, mode, total: 0, productTable: 0, uncertain: 0, prose: 0, unaudited: 0, empty: 0 };
    ...
    else if (row.verdict === "empty") t.empty++;
    else t.prose++;

    // empty never flags and never counts as unaudited - a zero-chunk document has nothing
    // retrievable, so there is nothing for either the gate or the census to have missed.
    if (mode === "enforce") {
