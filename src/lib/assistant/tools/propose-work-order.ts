...
export const proposeWorkOrderTool: AssistantTool = {
  ...
  description:
    "Author a NEW work order from natural language. Use this when the user wants cellar work assigned as a work order from specific instructions like 'Rack T12 to T15, add 30 ppm SO2, set T12 to 14C, clean and sanitize T15, and pull a panel.' The tool only proposes typed work-order tasks and returns a confirmation card; it never logs ledger operations, never completes tasks, and never creates materials. Supported task kinds: RACK and TOPPING (vessel to vessel); ADDITION/FINING (existing doseable material into a vessel); FILTRATION, CAP_MGMT (punchdown/pumpover), TEMP_SETPOINT; vessel maintenance CLEAN/SANITIZE/STEAM/OZONE/GAS/SO2/WET_STORAGE (any supply is overhead, never dosed into wine); the transform placeholders CRUSH/PRESS/HARVEST_WEIGH_IN; PANEL and BRIX observations; SAMPLE_PULL (pull/send a real lab sample on completion, optionally with a lab name and sendNow); and explicit checklist NOTE. Non-vessel equipment/floor cleaning and bottling are NOT supported here, and neither is group barrel-down / racking a whole barrel group in one order (author the barrel racks individually). Do not use rack_wine/add_addition/pull_sample for this when the user says work order or combines multiple planned tasks. IMPORTANT — transform tasks (CRUSH, PRESS/saignée, HARVEST_WEIGH_IN) are RUNTIME tasks: their real inputs are chosen by the crew on the execute screen, NOT baked into the work order. For PRESS this means the must lot, the source vessel, and the press fractions (cuts) are picked at execute time — so the tool does NOT bind a source vessel or lot, and the execute screen will read 'Nothing to press - no MUST lot is sitting in a vessel' until the crew selects the lot there. CRUSH is the same for its picks/destination, and HARVEST_WEIGH_IN for its block/weights. When the user names a source and/or destination vessel or a lot for a CRUSH or PRESS (e.g. 'press the lot in tank 6 into tank 5'), fold that guidance into that task's `note` field (e.g. note: 'Press tank 6 -> tank 5') so the crew sees it on the execute screen; do NOT drop that detail. AFTER proposing such a work order, tell the user plainly that the press/crush task's lot, source vessel, and fractions (or crush picks/destination) are selected by the crew on the execute screen, and that the task will show 'nothing to press/crush' there until they pick the lot — so they know the work order is fine, not broken.",
  kind: "write",
  inputSchema: {
    type: "object",
    properties: {
      ...
      tasks: {
        ...
        items: {
          ...
          properties: {
            ...
            title: { type: "string" },
            note: { type: "string", description: "Freeform guidance shown to the crew on the execute screen. For a CRUSH/PRESS task, put the source/destination vessels or lot the user named here (e.g. 'Press tank 6 -> tank 5') since those are chosen at execute time and are not otherwise bound." },
          },
          required: ["kind"],
        },
      },
