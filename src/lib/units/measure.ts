...
/** Resolve a possibly-aliased/cased unit string to a known canonical key, or null if unknown. */
export function resolveUnit(u: string | null | undefined): string | null {
...
export function dimensionOf(u: string | null | undefined): MeasureDimension | null {
  const k = resolveUnit(u);
  return k ? UNITS[k].dimension : null;
}
 ...
 */
export function convert(amount: number, from: string | null | undefined, to: string | null | undefined): number | null {
...
export function toCanonical(amount: number, unit: string | null | undefined): { amount: number; unit: "g" | "mL" | "unit" } | null {
  const dim = dimensionOf(unit);
  if (!dim) return null;
  const canonical = canonicalUnitFor(dim);
  const converted = convert(amount, unit, canonical);
