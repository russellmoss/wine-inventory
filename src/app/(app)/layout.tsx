...
import { AssistantObjectContextProvider, AssistantAvailabilityProvider } from "@/components/assistant/AssistantObjectContext";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const [pendingSamples, sparklingEnabled, complianceDeadlines, pendingWorkOrders, currency, unreadMessages, wineryTimeZone, customCrushEnabled, unitPrefs] = await Promise.all([
    ...
    effectiveTenantId ? getUnitPrefs() : Promise.resolve(DEFAULT_METRIC_PREFS),
  ]);
  ...
  return (
    <CurrencyProvider code={currency}>
      ...
      <AssistantAvailabilityProvider unavailableReason={assistantAvailability().unavailableReason}>
