...
import type { BlendVessel } from "@/lib/blend/data";
...
export function BlendBuilderClient({ vessels }: { vessels: BlendVessel[] }) {
  const router = useRouter();
  const occupied = vessels.filter((v) => v.residents.length > 0);

  const [selected, setSelected] = React.useState<Map<SourceKey, SelectedSource>>(new Map());
