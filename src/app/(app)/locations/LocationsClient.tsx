...
// `Location` is `vineyardScoped: false` in `entities.ts`, so create/rename/deactivate are all
// `adminAction` since the GLOBAL-1 fence. Gate the UI on the same predicate.
export function LocationsClient({ locations, isAdmin }: { locations: Loc[]; isAdmin: boolean }) {
    ...
    <div>
      ...
      {isAdmin ? null : (
        <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 24, maxWidth: "60ch" }}>
          An admin adds and renames locations. You can move stock between them from Inventory.
        </p>
      )}
        ...
        <table tabIndex={0} style={{ width: "100%", borderCollapse: "collapse", fontSize: 14.5 }}>
          <thead>
            <tr style={{ textAlign: "left", color: "var(--text-muted)" }}>
              ...
              {/* No row has an action for a non-admin, so the column would be dead weight. */}
              {isAdmin ? <th style={{ padding: "12px 16px", fontWeight: 500, textAlign: "right" }}>Actions</th> : null}
            </tr>
          </thead>
          <tbody>
            {locations.length === 0 ? (
              <tr>
                <td colSpan={isAdmin ? 3 : 2} style={{ padding: "20px 16px", color: "var(--text-muted)" }}>
                  {isAdmin ? "No locations yet. Add one above." : "No locations yet. An admin adds them."}
                </td>
              </tr>
            ) : (
              locations.map((loc) => (
