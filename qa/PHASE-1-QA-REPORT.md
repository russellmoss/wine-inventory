...
| 6 | swapLotCodes via UI | (swap core) | **SURFACE NOT IN BUILD** | — |
...
|-----|------|--------|----------------|
| Medium (gap) | **S6 — no swap UI** | `swapLotCodes` has a server action + core (guarded by `verify:naming`) but **no UI trigger** was built in Phase 1. Mislabeled-tank swaps can't be done from the UI. | Add a "swap codes" affordance (e.g., on lot detail or a two-lot picker) in a follow-up. Not a regression — swap was action-only by scope. |
| Low | **S9 — admin gating untestable here** | Template authoring is admin-gated server-side (`adminAction`); the demo user IS an admin, so the non-admin rejection path can't be exercised without a second (non-admin) user. | Confirm the "Admins only" rejection with a cellar-hand test user when role-switching (Phase 21a) exists. |
