---
title: NOW.md archive — everything closed out on or before 2026-08-05
status: historical record; do not work from this file
---

# NOW archive — through 2026-08-05

`NOW.md` is the working-set spine and is meant to fit on one screen. It had grown to **3,132 lines**:
roughly two dozen sessions' worth of `_Last updated` generations concatenated, plus every closed
ticket writeup since July.

**This file is that content, preserved verbatim and unedited.** Nothing was summarised, reordered or
dropped — the split was done by copying the whole file here and rebuilding a lean `NOW.md` beside it,
so there is no chance a section was lost in the cut. Headings and bodies drifted out of alignment
during the original concatenation (a block of closed-P0 detail sits under a `#586` heading, and
"Also in flight" appears twice); that misalignment is preserved too rather than guessed at.

Read it for history — why a decision went the way it did, what a fix actually changed, which
hypotheses were tested and refuted. Do not treat anything here as in flight.
