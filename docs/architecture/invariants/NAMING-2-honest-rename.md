...
**Guarded by:** `npm run verify:naming` (Phase 1) — verify-guarded **exactly like [[LEDGER-10-immutable-operations|LEDGER-10]]**: asserts a rename appends a `LotCodeEvent`, leaves every `LotOperationLine` snapshot untouched, and that historical/current reads resolve by `id`.
