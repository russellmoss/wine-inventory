...
| `src/lib/cellar/material-stock-core.ts` | receiveConsumableCore, adjustConsumableCore, transferConsumableCore | ✅ | `add-addition` | — |
| `src/lib/cellar/split-core.ts` | splitLotInPlaceCore | ✅ | `add-addition` | — |
| `src/lib/cellar/volume-correction-core.ts` | correctRecordedVolumeCore | ✅ | `add-addition` | — |
...
| `src/lib/compliance/transfer-in-bond-core.ts` | transferInBondCore, reverseTransferInBondCore | ✅ | `query-operations` | — |
| `src/lib/equipment/equipment-core.ts` | createEquipmentAssetCore, createEquipmentAssetsFromInvoiceCore | ✅ | `add-equipment` | — |
...
| `src/lib/gis/satellite/process-scene-core.ts` | processSceneJobCore | ✅ | `process-ndvi` | — |
| `src/lib/gis/satellite/scene-selection-core.ts` | searchScenesCore | ✅ | `process-ndvi` | — |
| `src/lib/grower/grower-core.ts` | createGrowerCore, updateGrowerCore | ✅ | `create-grower` | — |
| `src/lib/harvest/planned-harvest-core.ts` | setPlannedHarvestDateCore, retractPlannedHarvestDateCore, currentPlannedHarvestDatesCore, plannedHarvestDateAsOfCore, plannedHarvestChangesSinceCore | ❌ | — | internal |
| `src/lib/harvest/weigh-tag-core.ts` | createWeighTagCore, voidWeighTagCore | ✅ | `log-weigh-tag` | — |
| `src/lib/ingest/ingest-invoice-core.ts` | createIngestedInvoiceCore, updateIngestedInvoiceLineCore, updateIngestedInvoiceCore, setInvoicePaymentStatusCore, applyIngestedInvoiceCore, reverseIngestedInvoiceCore | ✅ | `ingest-documents` | — |
| `src/lib/ingest/manual-invoice-core.ts` | createManualInvoiceCore | ✅ | `add-invoice` | — |
| `src/lib/inventory/fg-cost-core.ts` | recordFinishedGoodReceiptCore | ✅ | `adjust-inventory` | — |
| `src/lib/owner/change-ownership-core.ts` | changeOwnershipCore, reverseChangeOwnershipCore | ✅ | `change-ownership` | — |
| `src/lib/owner/owner-core.ts` | createOwnerCore, updateOwnerCore | ❌ | — | internal |
| `src/lib/phenology/growth-core.ts` | estimateGrowthCore | ✅ | `query-field-reports` | — |
| `src/lib/phenology/stage-core.ts` | estimatePhenologyStageCore | ✅ | `query-field-reports` | — |
| `src/lib/plantingArea/migration-core.ts` | proposePlantingAreasFromBlocksCore, confirmProposedPlantingAreasCore | ❌ | — | internal |
| `src/lib/plantingArea/planting-area-core.ts` | createPlantingAreaCore, updatePlantingGeometryCore, oneBlockFromPlantingCore, splitIntoBlocksCore, assignBlockToPlantingCore, describePlantingStructureCore | ✅ | `describe-planting-structure` | — |
...
| `src/lib/spatial/block-metrics-core.ts` | computeBlockMetricsCore | ✅ | `process-ndvi` | — |
| `src/lib/spray/correction-core.ts` | correctSprayApplicationCore, voidSprayApplicationCore | ❌ | — | internal |
| `src/lib/spray/drying-override-core.ts` | recordDryingOverrideCore, recomputeDriedBeforeRainCore | ❌ | — | internal |
| `src/lib/spray/infection-read-core.ts` | composeInfectionStatusCore | ✅ | `query-spray-decision` | — |
| `src/lib/spray/legacy-mapping-core.ts` | recordLegacyMappingSuggestionCore, confirmLegacyMappingCore, rejectLegacyMappingCore | ❌ | — | internal |
| `src/lib/spray/record-core.ts` | recordSprayApplicationCore | ❌ | — | internal |
| `src/lib/transform/crush-core.ts` | crushLotCore | ✅ | `complete-task` | — |
| `src/lib/transform/press-core.ts` | pressLotCore | ✅ | `complete-task` | — |
| `src/lib/units/custom-unit-core.ts` | createCustomUnitCore, listCustomUnitsCore | ✅ | `query-custom-units` | — |
| `src/lib/vendors/vendor-import-core.ts` | acceptCandidateCore, rejectCandidateCore, mergeCandidateIntoVendorCore | ✅ | `create-vendor` | internal |
| `src/lib/vessels/group-core.ts` | getGroupDetailCore, listGroupDetailsCore, getGroupRollupsCore, getGroupMemberStatesCore, findGroupByNameCore, configureGroupCore, archiveGroupCore, addGroupMemberCore, removeGroupMemberCore, reorderGroupMembersCore | ✅ | `query-cellar-contents` | internal |
...
| `src/lib/weather/alert-core.ts` | detectWeatherAlertsCore, classifyForecastAlertsCore | ✅ | `query-climate` | internal |
| `src/lib/weather/forecast-hourly-read-core.ts` | composeForecastHoursCore | ✅ | `query-climate` | — |
| `src/lib/weather/forecast-ingest-core.ts` | ingestVineyardForecastCore | ❌ | — | internal |
| `src/lib/weather/forecast-read-core.ts` | composeForecastViewCore | ✅ | `query-climate` | — |
| `src/lib/weather/ingest-core.ts` | ingestVineyardWeatherCore | ❌ | — | internal |
| `src/lib/weather/rainfall-range-core.ts` | composeRainfallRangeCore | ✅ | `query-climate` | — |
| `src/lib/weather/read-core.ts` | composeClimateSummaryCore | ✅ | `climate-display` | — |
| `src/lib/weather/source-selection-core.ts` | selectPrimaryCore, gapFillCore, computeSpreadCore | ✅ | `climate-display` | — |
| `src/lib/work-orders/update-core.ts` | updateWorkOrderCore | ✅ | `complete-task` | — |

Coverage: **41/53** cores reachable by an assistant tool (13 internal, 2 deferred gap).

<!-- END GENERATED -->

## Replayed-history axis (plan 083)

Every eval in this repo used to start COLD — one synthetic user message, no prior conversation. That
made the whole suite blind to a class of failure it existed to police.

Measured on `claude-opus-4-8`, real system prompt, no `tool_choice`:

| Case | Cold | With replayed history (pre-fix) |
|---|---|---|
| `record_tasting_note` on a vessel | 10/10 | **0/8** |
| `log_brix` after 5 write turns | 5/5 | 2/5 |
| `propose_work_order` (the plan 081 repro) | 3/3 | 4/5 |

The mechanism: history was replayed as text only, so the model saw its own prior turns claiming
"review and confirm the card" with no tool call attached, and completed that pattern in prose. Fixed
by `src/lib/assistant/replay.ts`.

**What this means for adding a capability.** A cold golden case proves tool selection and nothing
about durability. When a write surface matters, add a `history` or `historyFixture` case to
`assistant-must-propose.golden.ts` as well. Two rules learned the expensive way:

1. **Depth is load-bearing.** At two prior write turns the pattern does not dominate and the case
   passes identically before and after the fix. Five is where it bites. Re-measure with
   `REPLAY_TEXT_ONLY=1` before assuming a history case guards anything.
2. **The fixture must not collide with the utterance.** A work-order case pointed at a transcript
   that already contained that work order, and the model correctly asked whether to duplicate it —
   scoring a failure for behaving well.

The eval routes through the shipped `buildReplayMessages`, so reverting the reconstruction turns
these cases red. `REPLAY_TEXT_ONLY=1` reproduces pre-fix behavior for comparison.
