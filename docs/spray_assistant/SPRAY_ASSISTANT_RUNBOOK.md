...
| S2b product facts master | 1B | 🟨 **FOUNDATION BUILT, PHASE NOT DONE — do NOT close.** Built + gates green: schema/RLS, the real resolver (LIVE, proven FRAC:7/11 on Pristine), pest ingest (41 categories / 42,132 mappings), coverage report, `verify:product-facts` 22/22, SPRAY-6, cron. 🟥 **NOT built: U1 jurisdiction is SCHEMA-ONLY (nothing writes the block-line snapshot → council C3 unfixed); U2 has no curated artifact/seeder/discipline-test; U3 has no `separation.ts` or oil↔sulfur goldens; U5 has no entry surface → SAFE-19 unprovable in-browser; U10 QA report unwritten.** Coverage 0% — both pre-committed thresholds BELOW, so **S7a and S6 stay blocked**. ⚠️ `prisma migrate diff` is UNSAFE here (proposes dropping tenant FKs repo-wide) | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | PR pending | [S2b QA](qa/S2b-qa-report.md) 🟨 foundation passes, 4 gate items deferred | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) |
