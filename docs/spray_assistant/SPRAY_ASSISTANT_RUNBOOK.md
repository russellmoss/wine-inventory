...
| S2b product facts master | 1B | 🟨 **FOUNDATION + Units 1/2/3/5 built, DB-proven (2026-07-27) — PHASE NOT DONE, do NOT close.** `separation.ts` (14 JMS-worked-example goldens), jurisdiction resolve+confirm+snapshot (`verify:spray-record` 15/15, council C3 closed in practice), the facts-artifact machinery (`seed-product-facts.ts`, live-tested against real CDPR — 1072 proposals reproducing the probe's own Pristine oracle exactly, artifact **not committed**, ships `[]`), and the tenant-facts entry surface (`/vineyards/sprays/products`, `verify:tenant-isolation` +6 cases) are all built and gate-green. 🔴 **New finding: the shipped Unit 6 resolver does not gate on `reviewedBy`** — every `verify:product-facts` fixture already relies on `reviewedBy: null` resolving, so real curated content must not be seeded until either the resolver is fixed to filter on it or the git PR review is explicitly treated as the sign-off gate (see [S2b-report.md](phases/S2b-report.md) Finding 1). Coverage still 0% — both pre-committed thresholds BELOW, so **S7a and S6 stay blocked** until a human curates real content (the §10 calibration spike is still unrun). ⚠️ No live interactive browser QA this pass — DB/script-proven only. ⚠️ `prisma migrate diff` is UNSAFE here (proposes dropping tenant FKs repo-wide) | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | [#535](https://github.com/russellmoss/wine-inventory/pull/535) MERGED + live · Units 1/2/3/5 on `claude/s2b-resume-units-2026-07-27`, not yet PR'd | [S2b QA](qa/S2b-qa-report.md) 🟨 updated 2026-07-27, 3 gate items closed, coverage + live-QA still open | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) · [S2b-report.md](phases/S2b-report.md) |
