...
| S2b product facts master | 1B | 🟨 **FOUNDATION MERGED + LIVE (#535, 2026-07-27) — PHASE NOT DONE, do NOT close.** Resolver proven in the LIVE request path (browser write → `REGISTRY` / `FRAC:7,11` / `PARTIAL`; toggle OFF → `NONE`). Built + gates green: schema/RLS, the real resolver (LIVE, proven FRAC:7/11 on Pristine), pest ingest (41 categories / 42,132 mappings), coverage report, `verify:product-facts` 22/22, SPRAY-6, cron. 🟥 **NOT built: U1 jurisdiction is SCHEMA-ONLY (nothing writes the block-line snapshot → council C3 unfixed); U2 has no curated artifact/seeder/discipline-test; U3 has no `separation.ts` or oil↔sulfur goldens; U5 has no entry surface → SAFE-19 unprovable in-browser; U10 QA report unwritten.** Coverage 0% — both pre-committed thresholds BELOW, so **S7a and S6 stay blocked**. ⚠️ `prisma migrate diff` is UNSAFE here (proposes dropping tenant FKs repo-wide) | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | [#535](https://github.com/russellmoss/wine-inventory/pull/535) MERGED + live | [S2b QA](qa/S2b-qa-report.md) 🟨 foundation passes, 4 gate items deferred | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) |
...
| S5a powdery index + latent ledger | 2C | 🟩 **ledger BUILT + verified; index NO-GO** (Unit 0 gate fired — all 8 sites failed; Units 3–4 cancelled, index moved to S5b behind S1) | [S5a plan v2](phases/S5a-powdery-index-latent-ledger-plan.md) | [S5a council](phases/S5a-council-feedback.md) | — | browser QA n/a — no UI yet (S9 owns presentation) | [S5a report](phases/S5a-report.md) · [Unit 0 probe](phases/S5a-diurnal-fidelity-probe.md) |
