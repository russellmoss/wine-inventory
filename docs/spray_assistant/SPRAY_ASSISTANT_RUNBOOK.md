...
| S2b product facts master | 1B | 🟨 **building — PR1 (schema slice, Units 0-5) BUILT + all gates green; Units 6-10 next.** ⛔ Tables ship EMPTY: curated content needs a HUMAN `reviewedBy` (rule §3.1), so the seeder writes proposals only. **[Unit 0 probe](phases/S2b-cdpr-interval-probe.md): CDPR `prod_site.dat` DOES carry per-crop PHI+REI — mancozeb's 66-day PHI reproduced. Nullity keyed on the UNIT column, NEVER the value (76% of PHI cells are `0`). Coverage 40.4/64.0/30.4% — below the pre-committed 80% gate, so S2b stays a CURATION phase with a seed.** ⚠️ `prisma migrate diff` is UNSAFE here (proposes dropping tenant FKs repo-wide — hand-author) | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | PR1 pending | — | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) |
