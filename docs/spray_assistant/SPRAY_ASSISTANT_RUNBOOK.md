...
| S2b product facts master | 1B | 🟦 **plan v2.3 — Unit 0 COMPLETE, rest ready for `/work`** (12 units; 5 critical defects folded; Unit 4 NARROWED — most shipped in #532). **[Unit 0 probe](phases/S2b-cdpr-interval-probe.md): CDPR `prod_site.dat` DOES carry per-crop PHI+REI — mancozeb's 66-day PHI reproduced. Nullity is keyed on the UNIT column, NEVER the value (76% of PHI cells are `0`). Coverage 40.4% PHI / 64.0% REI / 30.4% both — BELOW the pre-committed 80% gate, so S2b stays a CURATION phase with a seed, not an ingestion phase.** Pest codes = ingest of CDPR's 41 coarse categories; no species-level source exists | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | — | — | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) |
