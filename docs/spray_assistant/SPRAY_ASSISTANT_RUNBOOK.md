...
| S2b product facts master | 1B | 🟨 **BUILT — Units 0-9 done, all gates green; only Unit 10 (browser QA) remains and it needs RUSSELL to log in.** ⛔ Tables ship EMPTY: a curated row's `reviewedBy` is a human signature on a legal fact (rule §3.1), so the seeder writes proposals only — coverage report reads 0% by design. Resolver LIVE at the composition root (22/22). Pest vocabulary INGESTED (41 coarse categories, 42,132 mappings — categories NOT species; no public species source exists). ⚠️ `prisma migrate diff` is UNSAFE here — it proposes dropping tenant FKs repo-wide; hand-author migrations | [S2b plan v2.3](phases/S2b-product-facts-master-plan.md) | [S2b council](phases/S2b-council-feedback.md) | PR pending | Unit 10 pending | [Unit 0 probe](phases/S2b-cdpr-interval-probe.md) |
