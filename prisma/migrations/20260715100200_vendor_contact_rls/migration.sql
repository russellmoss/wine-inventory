-- Plan 069: RLS on vendor_contact (Phase-12 pattern): ENABLE + FORCE + a single tenant_isolation policy with
...
CREATE POLICY "tenant_isolation" ON "vendor_contact" USING ("tenantId" = current_setting('app.tenant_id', true)) WITH CHECK ("tenantId" = current_setting('app.tenant_id', true));

GRANT SELECT, INSERT, UPDATE, DELETE ON "vendor_contact" TO app_rls;
...
DECLARE
  r TEXT;
BEGIN
  FOREACH r IN ARRAY ARRAY['vendor_contact'] LOOP
    IF NOT EXISTS (
