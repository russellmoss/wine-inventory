...
- **Wave 4 built**: U14 setup-is-not-receipt (package size no longer seeds a lot — the phantom-50 bug) ·
  U15 receive in the unit you BUY in (pure `resolveReceiptQuantity` + `qtyUnit`; NO schema change, see below) ·
  U16 derived cost on each consumable row (read-only) · U17 vendor picker on the receipt screen (`vendorId`).
- ⚠️ **U15 deviated from the plan ON PURPOSE.** The plan wanted a new count dimension + pack-size column;
  both already exist (custom unit `perCanonical` IS the pack size, plan 075). A second pack-size column
  would be a competing source of truth for a number the cost engine divides by (COST-1). The real gap was
  the receive form having no unit selector at all.
- 🔎 **#366 and #374 are almost certainly SYMPTOMS OF U14**, not separate bugs: the phantom lot pinned the
