...
**Bug-report screenshot — exclude the "Report a bug" dialog + optionally the assistant from the capture. BUILT (this worktree `ecstatic-hofstadter-862254`), typecheck clean; pending browser-QA (needs interactive login).**
The assistant's "Report a bug" screenshot captured the report dialog itself (dim backdrop + "Report a bug ×" title bar) and the assistant, occluding the actual page. Fix, 3 files:
(1) [Modal.tsx](src/components/ui/Modal.tsx) — new `overlayProps` (`Record<\`data-${string}\`, string>`) spread onto the backdrop so the WHOLE modal can be excluded from a capture;
(2) [AssistantDock.tsx](src/components/assistant/AssistantDock.tsx) — tag the dock panel + FAB + expand-backdrop with `data-assistant-surface`;
(3) [FeedbackTicketModal.tsx](src/app/(app)/assistant/FeedbackTicketModal.tsx) — consent step gains an "Is the bug in the assistant?" Yes/No segmented control (default **No**) + a "Capture" button; the `html-to-image` filter ALWAYS drops `[data-feedback-capture-exclude]` (now the whole modal via overlayProps) and, when **not** in-assistant, ALSO drops `[data-assistant-surface]` (dock hidden → page behind it visible). On the full `/assistant` page nothing is tagged, so the toggle is a harmless no-op there. Also: `cacheBust:false` + a **15s timeout race** around `toPng` so a stalled capture degrades to the existing "you can still submit" fallback instead of an infinite "Capturing…".
**QA (in-app pane, Demo, logged in):** No-path proven with a real decoded PNG — clean dashboard, NO report popup, NO dock. Filter targeting proven deterministically (No→drop dock+modal / Yes→keep dock,drop modal / page always kept). BUT `html-to-image` hangs in this in-app pane once it's run several captures — the **dashboard No-capture that worked perfectly on the first clean run later hung on the identical code path**, so the hang is the documented "screenshots hang in this pane" limitation, NOT the feature code. The 15s guard makes it safe regardless.
**PENDING: user to sanity-check the capture (esp. the Yes path) in a REAL browser (rules out the pane), then /review → /ship.**
