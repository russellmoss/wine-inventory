...
(`MAX_CLARIFICATION_ROUNDS=2` now live). Watchdog + TTL sweep cron recovers strands. All 9 council concurrency fixes +
4 `/review` CRITICALs folded in. Browser-QA'd the complete loop in the in-app Claude browser (report → gate → DM →
chip → nudge → developer panel waiting→answered → reply → attempt-2 re-dispatch); QA fixtures cleaned, `.env` +
Demo `bugReportMode` restored. Gates green (tsc, eslint, vitest; CI check + tenant-isolation on #282).
