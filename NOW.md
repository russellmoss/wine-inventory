...
**Plan 079 winemaking KB RAG — 4 NEW SOURCES SHIPPED to PR #292 (branch `claude/kb-sources-osu-wsu-scott`); corpus 1,023→1,449 docs.**
Unit 12 re-crawl loop MERGED (#289). Added **WSU** (wine.wsu.edu, tier-1 auto-crawl, 95 docs — technical-only after
denylisting non-technical sections; sitemap at non-standard /wp-sitemap.xml → new `sitemapUrls` config), **OSU
Extension** (extension.oregonstate.edu, tier-1 curated, 36 docs — WINE/GRAPES ONLY via hub+sitemap discovery +
keyword filter), **OSU-OWRI** (ir.library.oregonstate.edu, tier-1 curated, 264 docs — Oregon Wine Research Institute
PDFs via ungated listing-walk avoiding the JS-challenge /concern/ pages), **Scott Labs** (scottlab.com, tier-2 VENDOR,
curated, 28 docs — 144pp winemaking handbook + curated wine articles). New config `sitemapUrls?` + `autoCrawl?`
(curated sources skip the weekly loop); `normalizeCrawlUrl` dedups /x.pdf vs /x.pdf/; `reset:knowledge-source` tool.
Robots/UA: our UA (CellarhandKnowledgeBot) is robots-'*'-permitted — OSU blocks only NAMED training crawlers
(ClaudeBot/GPTBot) w/ ai-train=no,use=reference; we do reference-use RAG (cite+link), not training. **Full crawl
live-proven (0 index errors); verify:knowledge-base 14/14 (8 AWRI/WA no-regression + rejection + 3 new-source cases);
tsc/naming/raw-sql green. PENDING: CI on #292 → merge. THEN Unit 11 per-tenant subscription settings UI (backend done).**
...
- **Plan 079 KB RAG — 4 new sources → PR #292** (WSU 95 + OSU Extension 36 + OSU-OWRI 264 + Scott Labs 28 = 426 new docs; corpus 1,449 total). Curated wine/grapes-only scoping; verify:knowledge-base 14/14.
